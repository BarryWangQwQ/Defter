...
import re as rgx
import os
import defter.backend.browsers as brw
...
_backend_js_file = pkg.resource_filename('defter.backend', 'defter.js')
_backend_js = open(_backend_js_file, encoding='utf-8').read()
_websockets = []
_call_return_values = {}
...
# Can be overridden through `backend.init` with the kwarg `js_result_timeout` (default: 10000)
_js_result_timeout = 10000

# All start() options must provide a default value and explanation here
_start_args = {
    'mode': 'auto',  # What browser is used
    'host': 'localhost',  # Hostname use for Bottle server
    'port': 0,  # Port used for Bottle server (use 0 for auto)
    ...
    'default_path': 'main.html',  # The default file to retrieve for the root URL
...
api_error_message = '''
----------------------------------------------------------------------------------
  'options' argument deprecated in v1.0.0, 
...
def expose(name_or_function=None):
    # Deal with '@backend.expose()' - treat as '@backend.expose'
    if name_or_function is None:
        return expose

    if type(name_or_function) == str:  # Called as '@backend.expose("my_name")'
...
# Examples: `backend.expose(w, "func_name")`, `backend.expose(func_name)`, `backend.expose((function (e){}), "func_name")`
EXPOSED_JS_FUNCTIONS = pp.ZeroOrMore(
    pp.Suppress(
        pp.SkipTo(pp.Literal('frontend.expose('))
        + pp.Literal('frontend.expose(')
        + pp.Optional(
...
def init(path='src', extensions=None, js_result_timeout=10000):
    time.sleep = sleep  # Monkey patch time.sleep() => sleep()
    if extensions is None:
        extensions = ['.html']  # allowed_extensions = ['.js', '.html', '.txt', '.htm', '.xhtml', '.vue']
    else:
        extensions.insert(0, '.html')
            ...
            try:
                with open(os.path.join(root, name), encoding='utf-8') as file:
                    ...
                    for expose_call in matches:
                        ...
                        msg = "frontend.expose() call contains '(' or '='"
...
def _backend():
    ...
    page = _backend_js.replace('/** _py_functions **/',
...
BOTTLE_ROUTES = {
    "/defter.js": (_backend, dict()),
    "/": (_root, dict()),
    "/<path:path>": (_static, dict()),
    "/frontend": (_websocket, dict(apply=[wbs.websocket]))
}
